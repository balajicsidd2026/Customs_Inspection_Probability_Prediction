...
tab1, tab2, tab3, tab4 = st.tabs(["Prediction", "EDA","Analytics Dashboard","Inspection Result"])
...
with tab2:
    ...
    with col1:
        ...
        ax_before.set_title(
            "Original Previous_Violations"
        )
        st.pyplot(fig_before)
        st.metric(
            "Skewness",
            f"{dataset['Previous_Violations'].skew():.2f}"
        )
        
    with col2:
        st.markdown("### After Log Transformation")
        dataset["Previous_Violations_Log"] = np.log1p(
            dataset["Previous_Violations"]
        )
        ...
        sns.histplot(
            dataset["Previous_Violations_Log"],
            kde=True,
            bins=20,
            ax=ax_after
        )
        ax_after.set_title(
            "Log Transformed Previous_Violations"
        )
        st.pyplot(fig_after)
        st.metric(
            "Skewness",
            f"{dataset['Previous_Violations_Log'].skew():.2f}"
        )
...
# ===================================================
# TAB 4 — BULK INSPECTION ANALYSIS
# ===================================================
